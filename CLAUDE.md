...
- The NAS MCP server has 118 shared tools in `ALL_TOOL_DEFS` but exposes only 5 per session (`tool_search`, `invoke_tool`, `run_command`, `check_disk_space`, `restart_nas_api`). `restart_nas_api` is implemented directly in `apps/nas-mcp/src/index.ts`, so `tools-config.json` has one more enabled entry than the shared registry. When debugging tool availability, check `tool_search` results and `tools-config.json` enablement — the count of definitions is not the count of exposed tools.
