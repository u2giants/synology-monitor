...
- The NAS MCP server has 119 shared tools in `ALL_TOOL_DEFS` but exposes only 5 per session (`tool_search`, `invoke_tool`, `run_command`, `check_disk_space`, `restart_nas_api`). `check_disk_space` and `restart_nas_api` are registry tools registered eagerly by `apps/nas-mcp/src/index.ts`. When debugging tool availability, check `tool_search` results, `tools-config.json` enablement, and `EAGER_TOOLS`.
