...
export const ALL_TOOL_DEFS: McpToolDef[] = [
...

  {
    ...
    description: "Read-only Snapshot Replication inspection with DSM 7 config/DB discovery: package state, daemon runtime, candidate schedule/retention config files, SQLite tables, snapshot counts, and recent prune/delete events. Does not start/cancel scrubs, create/delete snapshots, or run DSM tasks.",
    write: false,
    params: { target, filter },
    buildCommand: (input) => {
      const nameFilter = (input.filter as string | undefined)?.trim();
      ...
      return [
        ...
        "echo ''",
        "echo '=== SNAPSHOT / REPLICATION BINARIES DISCOVERED ==='",
        "for b in synobtrfs synobtrfsreplica synobtrfsreplicad synosnap synosnapshot synoshare synopkg; do",
        "  found=0",
        "  for p in /host/usr/syno/bin /host/usr/syno/sbin /host/usr/bin /host/usr/sbin /usr/syno/bin /usr/syno/sbin /usr/bin /usr/sbin /bin /sbin; do",
        "    [ -x \"$p/$b\" ] && { echo \"$b: $p/$b\"; found=1; break; }",
        "  done",
        "  [ \"$found\" -eq 0 ] && echo \"$b: not found\"",
        "done",
        "echo ''",
        ...
        `for v in /btrfs/volume[0-9]* /volume[0-9]*; do [ -d "$v/@Recently-Snapshot" ] || continue; echo "--- $v/@Recently-Snapshot ---"; find "$v/@Recently-Snapshot" -maxdepth 2 -type d 2>/dev/null${grepFilter} | head -40; done`,
        "echo ''",
        "echo '=== SNAPSHOT REPLICATION CONFIG PATH DISCOVERY ==='",
        "snapshot_roots='/host/usr/syno/etc /host/usr/syno/etc/packages /host/etc /host/packages/SnapshotReplication /host/packages/ReplicationService /host/log /var/packages/SnapshotReplication /var/packages/ReplicationService /volume1/@appdata/SnapshotReplication /volume2/@appdata/SnapshotReplication /volume1/@appdata/ReplicationService /volume2/@appdata/ReplicationService /volume1/@database/SnapshotReplication /volume2/@database/SnapshotReplication /volume1/@database/ReplicationService /volume2/@database/ReplicationService /volume1/@appconf/SnapshotReplication /volume2/@appconf/SnapshotReplication /volume1/@appconf/ReplicationService /volume2/@appconf/ReplicationService /volume1/@appstore/SnapshotReplication /volume2/@appstore/SnapshotReplication /volume1/@appstore/ReplicationService /volume2/@appstore/ReplicationService'",
        "echo 'Roots present:'",
        "for root in $snapshot_roots; do [ -e \"$root\" ] && echo \"  $root\"; done",
        "cfgs=$(",
        "  for root in $snapshot_roots; do",
        "    [ -d \"$root\" ] || continue",
        "    find \"$root\" -maxdepth 6 \\( -iname '*.conf' -o -iname '*.json' -o -iname '*.ini' -o -iname '*.cfg' -o -iname '*.db' -o -iname '*.sqlite' \\) 2>/dev/null",
        "  done | grep -Ei 'snapshot|replica|retent|sched|task|btrfs|share|folder|rotate|policy' | sort -u | head -80",
        ")",
        "if [ -z \"$cfgs\" ]; then echo 'No candidate Snapshot Replication config/DB files found in known DSM 7 roots.'; else echo \"$cfgs\"; fi",
        "echo ''",
        "echo '=== TEXT CONFIG SCHEDULE / RETENTION CLUES ==='",
        "text_cfgs=$(echo \"$cfgs\" | grep -Eiv '\\.(db|sqlite)$' | head -30)",
        "if [ -z \"$text_cfgs\" ]; then echo 'No candidate text config files found.'; fi",
        "for f in $text_cfgs; do",
        "  [ -f \"$f\" ] || continue",
        "  echo \"--- $f ---\"",
        `  grep -I -iE 'schedule|retention|rotate|rotation|policy|snapshot|replica|share|folder|keep|hour|daily|weekly|monthly|task|prune|delete|remove' "$f" 2>/dev/null${grepFilter} | head -80 || true`,
        "done",
        "echo ''",
        "echo '=== SNAPSHOT REPLICATION DATABASES ==='",
        "dbs=$(echo \"$cfgs\" | grep -Ei '\\.(db|sqlite)$' | head -20)",
        "if [ -z \"$dbs\" ]; then echo 'No Snapshot Replication SQLite DBs found in discovered config paths.'; fi",
        "for dbfile in $dbs; do",
        "  echo \"--- DB: $dbfile ---\"",
        "  echo 'tables:'",
        "  timeout 8 sqlite3 -readonly \"$dbfile\" '.tables' 2>&1 | head -20",
        ...
        "    timeout 8 sqlite3 -readonly \"$dbfile\" \"PRAGMA table_info(\\\"$tbl\\\");\" 2>&1 | head -40",
        "    echo \"--- table: $tbl sample rows ---\"",
        "    timeout 8 sqlite3 -readonly \"$dbfile\" \"SELECT * FROM \\\"$tbl\\\" LIMIT 10;\" 2>&1 | head -40",
        "  done",
        "done",
        "echo ''",
        "echo '=== SNAPSHOT REPLICATION WEBAPI DISCOVERY (READ ONLY) ==='",
        "if [ -z \"${DSM_USERNAME:-}\" ] || [ -z \"${DSM_PASSWORD:-}\" ]; then",
        "  echo 'DSM_USERNAME/DSM_PASSWORD not set; skipping Snapshot Replication WebAPI discovery.'",
        "else",
        "  DSM_BASE=\"http://localhost:${DSM_PORT:-5000}/webapi/entry.cgi\"",
        "  SID=$(curl -sfG \"$DSM_BASE\" --data-urlencode 'api=SYNO.API.Auth' --data-urlencode 'version=7' --data-urlencode 'method=login' --data-urlencode \"account=${DSM_USERNAME}\" --data-urlencode \"passwd=${DSM_PASSWORD}\" --data-urlencode 'format=sid' 2>/dev/null | grep -o '\"sid\":\"[^\"]*\"' | cut -d'\"' -f4)",
        "  if [ -z \"$SID\" ]; then",
        "    echo 'DSM WebAPI login failed; check DSM credentials/port.'",
        "  else",
        "    info=$(curl -sfG \"$DSM_BASE\" --data-urlencode 'api=SYNO.API.Info' --data-urlencode 'version=1' --data-urlencode 'method=query' --data-urlencode 'query=all' --data-urlencode \"_sid=$SID\" 2>/dev/null)",
        "    apis=$(echo \"$info\" | grep -o 'SYNO[^\"]*\\(Snapshot\\|Replication\\|Btrfs\\)[^\"]*' | sort -u | head -40)",
        "    if [ -z \"$apis\" ]; then echo 'No Snapshot/Replication-looking DSM APIs advertised.'; else echo \"$apis\"; fi",
        "    for api in $apis; do",
        "      case \"$api\" in",
        "        *Snapshot*|*Replication*|*Btrfs*) ;;",
        "        *) continue ;;",
        "      esac",
        "      for method in list get enum; do",
        "        echo \"--- $api.$method ---\"",
        "        curl -sfG \"$DSM_BASE\" --data-urlencode \"api=$api\" --data-urlencode 'version=1' --data-urlencode \"method=$method\" --data-urlencode 'offset=0' --data-urlencode 'limit=200' --data-urlencode \"_sid=$SID\" 2>/dev/null | head -c 8000",
        "        echo ''",
        "      done",
        "    done",
        "    curl -sfG \"$DSM_BASE\" --data-urlencode 'api=SYNO.API.Auth' --data-urlencode 'version=7' --data-urlencode 'method=logout' --data-urlencode \"_sid=$SID\" >/dev/null 2>&1 || true",
