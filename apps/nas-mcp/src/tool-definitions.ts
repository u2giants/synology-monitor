...
export const ALL_TOOL_DEFS: McpToolDef[] = [
...

  {
    ...
    description: "Read-only latency sample across all SATA disks (or a specific disk via filter, e.g. '/dev/sda' or 'sda'). Phase 1: 256 MB sequential O_DIRECT read — throughput + avg sequential latency from diskstats (65 K ops). Phase 2: 100 random single-block reads scattered across the full disk extent — diskstats avg (kernel-accurate, no process overhead) plus wall-clock distribution (min/avg/p95/max, includes ~3-5 ms dd startup overhead per op). Zero writes.",
    write: false,
    params: { target, filter },
    buildCommand: (input) => {
